...
class CoverageHHTableByPerfecture extends Chart
{
      public function getData(string $filterPath = ''): Collection
    {
        ...
        $data =  (new BreakoutQueryBuilder('listing'))
            ->select(["LPAD(SUBSTRING(cons_zd_id, 1, 2), 2, '0') area_code_prefecture",
                      "SUM(case when cons_is_deleted <> 3 or cons_is_deleted is null then  CONS_MEN_ORD_CARTO else 0 end) as carto", 
                      "SUM(case when cons_is_deleted <> 2 or cons_is_deleted is null then CONS_MEN_ORD_CENSUS else 0 end) as existant",
