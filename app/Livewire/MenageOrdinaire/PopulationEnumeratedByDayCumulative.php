...
class PopulationEnumeratedByDayCumulative extends Chart
{
    public function getData(string $filterPath = ''): Collection
    {
        $data = (new BreakoutQueryBuilder(dataSource: $this->indicator->data_source, filterPath: $filterPath))
            ->select(["DATE_FORMAT(FROM_UNIXTIME(i15), '%Y-%m-%d')
                AS enumeration_date", 'COUNT(*) AS total_pp'])
            ->from(['sect01','sect02'])
        ...
        $totalTarget = (new AreaTree())->areas($filterPath, referenceValueToInclude: 'population')->sum('ref_value');
        ...
        $merged = $allDates->map(function ($date) use ($targetsKeyed, $dataKeyed, &$runningTotal,&$runningDailyTarget) {
            ...
            if ($dataItem = $dataKeyed->get($date)) {
                $row['total_pp'] = $dataItem->total_pp;
            }
            $runningTotal += $row['total_pp'];
