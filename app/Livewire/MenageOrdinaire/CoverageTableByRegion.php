...
class CoverageTableByRegion extends Chart
{
  
    public function getData(string $filterPath = ''): Collection
    {
        $data =  (new BreakoutQueryBuilder('listing', $filterPath))
            ->select(["SUM(case when cons_is_deleted <> 3 or cons_is_deleted is null then  1 else 0 end) as carto", 
    ...
    public function getTraces(Collection $data, string $filterPath, bool $designMode = false): array
    {
        ...
        $header = [
            ['Nom de region', 'Couverture par rapport aux concessions Carto (%)', ' Couverture par rapport aux concessions existantes (%)']
        ];
        $cells = [
