...
namespace App\Livewire\Concession;
...
class PercentageOfPopulationListedAgainstTargetByArea extends Chart
{
    ...
    public function getData(string $filterPath = ''): Collection
    {
        try {
            $data =  (new BreakoutQueryBuilder($this->indicator->data_source, $filterPath))
                ->select(["SUM(CONS_SIZE_CONC) AS total"])
                ->from(['cons_infra_rec'])
                ->where(['CONS_IS_DELETED in (1,3)'])
                ->groupBy(['area_code'])
                ->lastlyAreaLeftJoinData(referenceValueToInclude: 'population')
                ->get();
                $aggregatedValue = $data->sum('total');
                $aggregatedRefValue = $data->sum('ref_value');
                $data[] = (object)[
                    'area_code'=> '0',
            ...
            $data = $data->map(function ($area) use ($day_count_yesterday, $days_Since_enum_start, $total_Enum_days) {
                ...
                $area->expected_no = safeDivide($days_Since_enum_start, $total_Enum_days) * $area->ref_value;
                $area->expected_no_yesterday = safeDivide($day_count_yesterday, $total_Enum_days) * $area->ref_value;
