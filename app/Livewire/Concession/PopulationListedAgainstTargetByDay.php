...
class PopulationListedAgainstTargetByDay extends Chart
{
    public function getData(string $filterPath = ''): Collection
    {
        $data = (new BreakoutQueryBuilder(dataSource: $this->indicator->data_source, filterPath: $filterPath))
            ->select(["DATE_FORMAT(FROM_UNIXTIME(CONS_CONC_DATE), '%Y-%m-%d')
                AS enumeration_date", 'SUM(CONS_SIZE_CONC) AS total_pp', '0 AS ref_value', 'NULL AS dailytarget', 'SUM(CONS_SIZE_CONC) AS dailyPerformance'])
            ->from(['cons_infra_rec'])
        ...
        for($i = 1; $i <= $total_enum_days; $i++) {
            ...
            if ($data->where('enumeration_date',$enumeration_date )->isEmpty()) {
                ...
                $row->dailyPerformance = $row->total_pp;
            }
        }
