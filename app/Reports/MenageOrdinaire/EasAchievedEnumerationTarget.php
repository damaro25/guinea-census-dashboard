...
class EasAchievedEnumerationTarget extends ReportBaseClass
{
    public function getData(string $filterPath): Collection
    {
        ...
        $sql = "SELECT full_path AS `EA code`, EA.REGION,EA.PREFECTURE AS `PRÉFECTURE`,EA.COMMUNE AS `SOUS-PRÉFECTURE/COMMUNE`,EA.`ZS`,EA.`ZC`,D.enum_hh,path FROM
                ( ". $sql . ") D
        ...
        $data = $data->map(function ($row) {
            ...
            ->only(['REGION','PRÉFECTURE','`SOUS-PRÉFECTURE/COMMUNE`','ZS','ZC','EA', 'Interviewer Name', 'Enumerated HHs', 'Target', 'Performance(%)'])
                ->all();
        });
