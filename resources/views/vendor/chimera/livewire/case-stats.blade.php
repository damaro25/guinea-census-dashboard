            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                @foreach($stats ?? [] as $name => $value)
                    <div class="flex rounded-md shadow bg-white p-2">
                        <x-chimera::case-icon :type="$name" class="opacity-75 h-14" />
                        <div class="flex-1 truncate px-4 h-38">
                            <div class="font-medium text-2xl text-gray-900 hover:text-gray-600">{{(new \NumberFormatter(app()->getLocale(), \NumberFormatter::TYPE_INT32))->format((float)$value) }}</div>
                            <div class="text-gray-500 text-sm text-wrap whitespace-normal">{{ ucfirst(__($name)) }}</div>
                        </div>
                    </div>
                @endforeach
            </div>
